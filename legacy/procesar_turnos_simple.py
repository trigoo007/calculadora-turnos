...
Maneja la lógica simplificada de días completos según el día del turno
"""
...
def obtener_dias_turno(fecha_turno):
    """
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Uso: python3 procesar_turnos_simple.py <ruta_archivo_csv> [directorio_salida] [nombre_doctor]")
    ...
    while True:
        ...
        try:
            ...
            fechas_turno.append(fecha_turno)
            
            # Obtener los días a incluir para este turno
            dias, horas = obtener_dias_turno(fecha_turno)
            total_horas += horas
            ...
            dia_semana = ['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'][fecha_turno.weekday()]
            print(f"Añadido turno del {dia_semana} {fecha_turno.strftime('%d-%m-%Y')} ({horas} horas)")
