...
pero considerando todos los exámenes - Versión corregida para incluir TACs especiales
"""
...
def clasificar_examenes_personalizado(calculadora):
    """
    Sobrescribe la clasificación de exámenes con una versión que reconoce más tipos de TAC doble.
    """
    if calculadora.data_filtrada is None:
        return False
    
    try:
        # Clasificar como RX o TAC
        calculadora.data_filtrada['Tipo'] = 'RX'
        mask_tac = calculadora.data_filtrada['Nombre del procedimiento'].str.contains('TAC', case=False, na=False)
        calculadora.data_filtrada.loc[mask_tac, 'Tipo'] = 'TAC'
        
        # Criterios oficiales para TAC doble
        tac_dobles_oficiales = [
        ...
        mask_tac_doble_oficiales = calculadora.data_filtrada['Nombre del procedimiento'].isin(tac_dobles_oficiales)
        
        # Identificar TAC doble según criterios adicionales
        mask_tac_doble_adicionales = False
        for criterio in tac_dobles_adicionales:
            mask_tac_doble_adicionales |= calculadora.data_filtrada['Nombre del procedimiento'].str.contains(criterio, case=False, na=False)
        
        # Combinar todas las condiciones para identificar TAC doble
        calculadora.data_filtrada['TAC doble'] = mask_tac_doble_oficiales | mask_tac_doble_adicionales
        
        # Log de la clasificación
        print("\nExámenes clasificados como TAC doble:")
        tac_dobles = calculadora.data_filtrada[calculadora.data_filtrada['TAC doble']]
        for idx, exam in tac_dobles.iterrows():
            print(f"- {exam['Número de cita']}: {exam['Nombre del procedimiento']}")
        
        print(f"\nTotal de TAC dobles clasificados: {len(tac_dobles)}")
        
        return True
    except Exception as e:
        print(f"Error al clasificar exámenes personalizados: {e}")
        return False

def main():
    """Función principal que procesa un archivo CSV con días de turno específicos."""
    if len(sys.argv) < 3:
        print("Uso: python3 procesar_turnos_completo_fixed.py <ruta_archivo_csv> <fecha1>[,F] [<fecha2>[,F] ...] [output_dir] [nombre_doctor]")
        print("Ejemplo: python3 procesar_turnos_completo_fixed.py data.csv 08-abr-2025 09-abr-2025,F")
    ...
    try:
        ...
        # Clasificar exámenes con la función personalizada 
        if not clasificar_examenes_personalizado(calculadora):
            print("Error al clasificar los exámenes personalizados")
            sys.exit(1)
        ...
        print(f"\nExámenes TAC: {eco['tac_count']} (${eco['tac_total']:,})")
        print(f"\nExámenes TAC doble: {eco['tac_doble_count']} (${eco['tac_doble_total']:,})")
